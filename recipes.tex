...
\cs{mkbibnamefirst} (which prints the initials or first names),
\cs{mkbibnameprefix} (which prints the `de' or `von' parts),
\cs{mkbibnamelast} (which prints the last name) and
\cs{mkbibnameaffix} (which prints the `Jr' part). Each or all of these
...
\end{pseudoverb}
Where \angled{order} can be |first-last| (for ordinary order),
