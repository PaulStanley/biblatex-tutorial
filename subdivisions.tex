...
\printbibliography[heading=subbibliography,
                   ...
                   filter=secondary:sources]
\end{verbatim}
We use \cs{printbibheading} simply as a convenience to print a standard `top level' heading for the whole bibliography, but then use the |heading| and |title| options when actually printing the bibliography. As far as Biblatex is concerned, a document
...
a standard pattern: 
\begin{quote} 
\angled{descriptor} \texttt{=} \angled{value}
\end{quote}
...
\begin{quote}
\texttt{not}\angled{descriptor} \texttt{=} \angled{value}
\end{quote}
...
\newthought{The easiest type of choice} to build in to a bibliography is
...
\item You can enter the information in the \emph{document source
  ...
  |\DeclareBibliographyCategory{crucial}| (2) You then add individual
...
So, as you can see, both keywords and categories are ways of arbitrarily
...
\item
  ...
  |\addbibresource{}| to identify each resource, but make use of the
...
\end{verbatim}
\item  
  Use explicit refsection commands.\footnote{Either \cs[refsection]{begin} and \cs[refsection]{end} or \cs{newrefsection}.} As each section begins, include a
...
Finally, because a possible ``use case'' is to loop through each section
