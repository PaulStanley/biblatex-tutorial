...
source code.\sidenote{Especially \texttt{standard.def} and the
  ...
  files.} This document is not trying to teach you how to do that. But
...
\indexstart{customization!general techniques}
...
instance \cs{AtEveryCitekey} allows one to execute some code whenever
a source is about to be cited, at a time when its data is available
but has not yet been handled; \cs{AtEveryBibitem} offers a similar
facility in relation to the printing of bibliography
items. Particularly useful, sometimes, is the hook \cs{AtDataInput},
...
\indexstop{customization!general techniques}

\section{Where to put customizations}

\indexstart{customization!file locations}
...
\item most other customizations should go in the document preamble, after
  ...
  I specifically mention it.\index{customization!in preamble}
\end{itemize}
\item If you want a standard set of options to apply to all your
  ...
  must do when you load \biblatex.\index{customization!biblatex.cfg@\texttt{biblatex.cfg}}\index{files!biblatex.cfg@\texttt{biblatex.cfg}}
...
\end{itemize}
\indexstop{customization!file locations}
