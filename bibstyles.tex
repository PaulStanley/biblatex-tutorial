...
The next big division is between \emph{abstract} labelling systems and
...
8, 10, 7, 6, 5]} may place numbers in brackets, or
...
text. \marginnote{(Author 1980) (Author: 1980) (Author, 1980)}Author/year
systems may format their references in a variety of different
ways. And full-bibliographical systems differ tremendously\marginnote{supra n. $\theta$, n $\theta$ above, (n $\theta$)} in the
...
\package{Biblatex} styles also fall into these three families, and there are
a number of `standard' styles in each. In principle, \biblatex\
distinguishes between the `citation style'\marginnote{\cs{usepackage[citestyle=...]\{biblatex\}}} (which determines how
citations appear in the text) and the `bibliography style'\marginnote{\cs{usepackage[bibstyle=...]\{biblatex\}}} (which
...
\biblatex\, which will load both citation style and bibliography style. That is what we assume here.
...
provide accurate implementations of particular detailed guides. In principle, there is much to be said for specific styles, which are more likely to be accurate reflections of citation practices in the field to which they relate.
...
\verb|sorting=none|
\end{center}
when you load \biblatex.\intref{See chapter~\ref{ch:sorting} for much more information on sorting.}
...
\item If you want references \emph{uncompressed} and \emph{not
    reordered}, just load the style \verb|numeric|. The package will then print the references you give in any citation in exactly the order you cited them in.
\item If you want references \emph{sorted and re-ordered} but
  \emph{not} compressed, then load the style \verb|numeric|, but also
  add the option |sortcites=true|. (This, at least, you will surely nearly always want!)
...
For \emph{superscript citations}, you have three options:
\begin{enumerate}
\item Use the \cs{supercite} command\marginnote{\texttt{blah.}\cs{supercite\{a\}}
    $\rightarrow$blah.$^{1}$} instead of \cs{cite}: this will
...
Changing the \emph{punctuation between multiple citations} is
easy. By default it's a comma. You change it by redefining
\cs{multicitedelim}.\intref{See further, chapter \ref{ch:recipes}.} So if you want a semicolon, you would put (in
your preamble)
\begin{center}
...
bold\marginnote{[\textbf{1}, \textbf{2}]} or some other
...
work. This topic is covered in some detail below.\intref{Page \pageref{recipe:brackets}.}
...
brackets\marginnote{\cs{cite\{a, b\}}\gives [AA01, BB02]}
separated by commas
whereas with \verb|alphabetic-verb| you get each reference in its own
set of brackets, separated by semicolons.\marginnote{\cs{cite\{a,
    b\}}\gives [AA01]; [BB02]}
...
covered in this document: consult the \biblatex\ documentation.\intref{\emph{Manual} \S\ 4.5.4}
...
\newthought{Besides selecting} from the range of styles,
  there's not much room for simple customization; but there are three
things you may wish to adjust.

First, the number of names that get printed.\intref{See further, page \pageref{recipes:maxnames}.}
...
A second adjustment you might want to make is to punctuation:\intref{See generally, chapter \ref{ch:recipes}.}
\begin{itemize}
...
\item The separator between author and title is usually a comma. To
  change it, redefine \cs{nametitledelim}. For instance, to change it
  to a colon,
\begin{verbatim}
...
\item If you are using ibid,\intref{Page \pageref{ibidtracking}.} you can configure it using the options
  given above.
\end{itemize}
...
Standard\marginnote{The standard verbose styles are, I think, not
  ...
  point for customization.} \biblatex\ offers no fewer than seven different flavours of
...
\package{biblatex-dw} bundle, the \package{biblatex-chicago}
package, with option \verb|notes| and the \package{oscola} style ---
